...
export default function NewTeachingPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!referencia.trim()) errors.push({ field: 'referencia', message: 'Referencia e obrigatoria' });
    ...
    if (!duracaoEstimada || Number(duracaoEstimada) <= 0) {
      errors.push({ field: 'duracao_estimada_minutos', message: 'Duracao deve ser maior que 0' });
    }
    ...
    if (segmentos.length === 0) {
      setFieldErrors([{ field: 'segmentos_de_texto', message: 'Insira pelo menos um paragrafo de conteudo' }]);
      return;
    }
    ...
    try {
      await api.post<TeachingResponse>('/api/teachings/generate', {
        titulo: titulo.trim(),
        categoria,
      ...
      showToast('Ensinamento criado com sucesso!', 'success');
      ...
      setError(data?.message || 'Erro ao criar ensinamento. Tente novamente.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground">Novo Ensinamento</h1>
        <p className="mt-1 text-sm text-muted">
          Crie um audiobook baseado em um ensinamento bíblico
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input
              ...
              placeholder="Ex: Sermão do Monte"
              disabled={isLoading}
            />
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: Mateus 5:1-12; Lucas 6:20-26"
              disabled={isLoading}
            />
          ...
          <div>
            <label htmlFor="duracao-estimada" className="mb-1.5 block text-sm font-medium text-muted">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Escreva o texto do ensinamento. Separe os segmentos com uma linha em branco entre eles.

Ex:
Bem-aventurados os pobres de espirito, porque deles e o Reino dos ceus.

Bem-aventurados os que choram, porque eles serao consolados.

Bem-aventurados os mansos, porque eles herdarao a terra."
              disabled={isLoading}
            />
